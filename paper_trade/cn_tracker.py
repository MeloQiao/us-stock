"""
A-Stock Virtual Portfolio Tracker

Local JSON replacement for the HuggingFace-based VirtualPortfolio.
Tracks virtual CNY 100万 positions driven by daily strategy signals.

Usage
─────
  # Show current virtual portfolio
  python3 paper_trade/cn_tracker.py show

  # Reset to fresh start (CNY 100万 cash, no positions)
  python3 paper_trade/cn_tracker.py reset
"""
...
_DATA_FILE = Path(__file__).parent / "cn_virtual.json"

TOTAL_CAPITAL = 1_000_000   # CNY 100万 virtual capital
CURRENCY      = "CNY"
...
def _fresh_state() -> dict:
    return {
        "_note": "A-stock virtual portfolio. Auto-managed by cn_tracker.py via daily signals.",
...
def _invested(data: dict) -> float:
    return sum(p["notional"] for p in data["positions"])
...
def execute_signals(
    ...
    for sym, sig in signals.items():
        ...
        logger.info("[cn] SELL %s @ %.2f  pnl=%.0f (%.1f%%)", sym, exit_price, pnl, pnl_pct)
    ...
    if new_buys and _available_cash(data) > 0:
        ...
        for sym in new_buys:
            ...
            logger.info("[cn] BUY  %s @ %.2f  notional=%.0f  score=%.1f",
...
def format_portfolio(prices: Optional[dict[str, float]] = None) -> str:
    ...
    lines = [f"📋 A股虚拟仓  (as of {load()['updated']})"]
    lines.append(
        f"   总资本 ¥{total:,.0f}  现金 ¥{cash:,.0f}  "
        f"持仓 ¥{inv:,.0f}  浮盈 ¥{unr:+,.0f}  已实现 ¥{rlz:+,.0f}"
    )
    lines.append("")
    if s["open_positions"]:
...
def _cli():
    ...
    if cmd == "show":
        ...
        save(_fresh_state())
        print("✅ A股虚拟仓已重置 — 资本 ¥1,000,000，空仓")
        print(format_portfolio())
